    ...
    <div class="span12">
        <div class="grid simple horizontal red">
            <div class="grid-title">
                <h3 class="text-danger"><i class="fa fa-file text-danger"></i> รายละเอียดการรับสินค้าแบบไม่มีใบสั่งซื้อ (收货明细)</h3>
                    ...
                    <div class="table-responsive">
                        <table class="table  table-bordered table-responsive" width="100%">
                            ...
                            <tr v-for="(model,index) in dataDetails">
                                ...
                                <td class="text-right">{{model.quantity*model.unit_price}}</td>
                                <td><input type="text" min="0" v-model="model.amount" class="input-sm "></td>
                                <td><input type="text" min="0" v-model="model.weight" class="input-sm "></td>
                            </tr>
                            <tr class="warning">
                                <td colspan="3" class="text-center footer-total" >ยอดรวม</td>
                                <td    class="text-right footer-total">{{SumUnit}}</td>
                                <td  colspan="2"  class="text-right footer-total">{{SumPrice}}</td>
                                <td class="text-right footer-total">{{SumAmount}}</td>
                                <td class="text-right footer-total">{{SumWeight}}</td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
...
filters:{
   ...
   computed:{
      ...
      var order_details=this.dataDetails;
        ...
        for (var i = 0; i < order_details.length; i++) { 
            total_price +=parseFloat(order_details[i].quantity)*parseFloat(order_details[i].unit_price) ;
         }
            Total=total_price -this.deposit;
            return Total
      },
      SumAmount:function(){
      var totalamount = 0 ;
      var order_details=this.dataDetails;
        for (var i = 0; i < order_details.length; i++) { 
         var amount = order_details[i].amount;
      ...
      var order_details=this.dataDetails;
        for (var i = 0; i < order_details.length; i++) { 
         var weight = order_details[i].weight;
            totalWeight +=parseFloat(weight);
            }
           
            return isNaN(totalWeight)?0:totalWeight.toFixed(2);
      },
   },
    methods:{
